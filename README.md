# Overview
This is a simple Hello World application written in JavaScript that displays a greeting message to the console.

The purpose of this software is to demonstrate my ability to set up a development environment, write clean code, and publish projects to GitHub while following professional software development practices.

[Software Demo Video]()

# Development Environment

* Visual Studio Code
* Node.js
* Git
* GitHub

The programming language used is JavaScript (Node.js runtime).

# Useful Websites

* [MDN Web Docs - JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
* [Node.js Documentation](https://nodejs.org/en/docs/)
